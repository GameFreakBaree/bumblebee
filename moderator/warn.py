...
class ModWarn(commands.Cog):
    ...
    async def warninfo(self, ctx, *, member: discord.Member):
        ...
        if command_status == 1:
            ...
            bumblebee_cursor.execute(f"SELECT * FROM bumblebee_warns WHERE guild_id = {ctx.guild.id} AND user = {member.id} LIMIT 5")
            warn_id_tuple = bumblebee_cursor.fetchall()
            ...
            warn_message = await ctx.send(embed=warn_embed)

            if doorgaan:
                await warn_message.add_reaction("◀️")
                await warn_message.add_reaction("▶️")
                ...
                while True:
                    try:
                        ...
                        if str(reaction.emoji) == "▶️" and page != max_rows:
                            page += 1
                            await warn_message.remove_reaction(reaction, member)
                            nieuwe_pagina = True
                        ...
                        if nieuwe_pagina:
                            ...
                            bumblebee_cursor.execute(f"SELECT * FROM bumblebee_warns WHERE guild_id = {ctx.guild.id} AND user = {member.id} LIMIT {offset}, 5")
                            warn_id_tuple = bumblebee_cursor.fetchall()
                            ...
                            warn_embed.set_author(name=f"[WARNINGS] {member}", icon_url=member.avatar_url)
                            warn_embed.set_footer(text=f"{footer} | Page {page}/{max_rows}")
                            await warn_message.edit(embed=warn_embed)
                    except asyncio.TimeoutError:
                        await warn_message.clear_reaction("◀️")
                        await warn_message.clear_reaction("▶️")
                        break
        db_bumblebee.close()
